...
router.post('/', async (req, res) => {
  ...
  try {
    // Verifica se já existe user igual
...
router.put('/update-por-id/:id', async (req, res) => {
  try {
    // Se tentar alterar o user, verifica se já existe outro igual
    if (req.body.user) {
...
router.put('/update-por-nome/:name', async (req, res) => {
  try {
    ...
    const updated = await Usuarios.findOneAndUpdate({ name: req.params.name }, req.body, { new: true });
    if (!updated) {
      return res.status(404).json({ erro: "Nenhum usuário com esse nome foi encontrado!" });
    }
    res.json(updated);
...
router.put('/update-por-user/:user', async (req, res) => {
  try {
    ...
    const updated = await Usuarios.findOneAndUpdate({ user: req.params.user }, req.body, { new: true });
    if (!updated) {
...
router.delete('/delete-por-user/:user', async (req, res) => {
  try {
    ...
    if (!deleted) {
      return res.status(404).json({ erro: "Não foi possível deletar o usuário com o nome de usuário especificado!" });
    }
    res.json(deleted);
  } catch (err) {
    res.status(500).json({ error: err.message });
